...
{
    AddSubscriber<MapCoordinatesTopic>([&data_source = data_source_](const auto& map_coordinates)
                                       { data_source.SetMapCoordinates(map_coordinates); });
    AddSubscriber<PreviousPathTopic>([&data_source = data_source_](const auto& previous_path)
                                     { data_source.SetPreviousPath(previous_path); });
    ...
    AddSubscriber<SpeedLimitTopic>([&data_source = data_source_](const auto& speed_limit)
                                   { data_source.SetSpeedLimit(speed_limit); });
    AddSubscriber<SensorFusionTopic>([&data_source = data_source_](const auto& sensor_fusion)
                                     { data_source.SetSensorFusion(sensor_fusion); });
}

void MotionPlanningNode::InitPublisher()
{
    AddPublisher<SelectedTrajectoryTopic>([&motion_planning = motion_planning_]
                                          { return motion_planning.GetSelectedTrajectory(); });
}
